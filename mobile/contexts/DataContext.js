...
export function DataProvider({ children }) {
  ...
  const refreshData = useCallback(async () => {
    ...
    if (isSandbox) {
      ...
      const localReminders = await AsyncStorage.getItem("kylr_reminders");
      let loadedReminders = [];
      if (localReminders) {
        loadedReminders = JSON.parse(localReminders);
      } else {
        await AsyncStorage.setItem("kylr_reminders", JSON.stringify([]));
      }
  ...
  const addReminder = async (data) => {
    if (isSandbox) {
      ...
      await AsyncStorage.setItem("kylr_reminders", JSON.stringify(updated));
      return { success: true };
    } else {
      try {
        const postUrl = `${appsScriptUrl}?action=addReminder&uid=${user.uid}`;
        const body = {
          title: data.title,
          amount: parseFloat(data.amount) || 0,
  ...
  const deleteReminder = async (remId) => {
    if (isSandbox) {
      ...
      try {
        const postUrl = `${appsScriptUrl}?action=deleteReminder&uid=${user.uid}&reminderId=${remId}`;
        const res = await fetch(postUrl, {
          method: "POST",
          headers: { "Content-Type": "text/plain" }
        });
  ...
  const checkOffReminder = async (remId, monthStr) => {
    if (isSandbox) {
      ...
      await AsyncStorage.setItem("kylr_reminders", JSON.stringify(updatedReminders));
      ...
      try {
        const postUrl = `${appsScriptUrl}?action=checkOffReminder&uid=${user.uid}&reminderId=${remId}&monthStr=${monthStr}`;
        const res = await fetch(postUrl, {
          method: "POST",
          headers: { "Content-Type": "text/plain" }
        });
